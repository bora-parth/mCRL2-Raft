...
cd /scratch/20201025
mcrl22lps raft_spec_AC_12.mcrl2 raft_AC_12.lps

echo "going tmux mode rn"
# Iterate over the files and create tmux sessions
for file in "${files[@]}"; do
    session_name="${file%.*}_crash_12" # Extract the session name without the file extension
    ...
    tmux send-keys -t "$session_name" "lps2pbes -v raft_AC_12.lps -f $file raft_$session_name.pbes" Enter
